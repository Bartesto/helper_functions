...
dir="C:\\temp\\R_development\\l87"
zone=51
shp="selected_rainforest_94_extents_2012_for_report.shp"
shp.ID="rf_id"
pr=10970
option="b2"
sensor = "l8"
...
# folds.no.7 <- fold[!nL7.index]
        ...
        for (i in 1:length(imdate)){
                
#                 dir.i <- dirlistLong[i]
#                 setwd(dir.i)
                data.i <- paste0(dir,"/",result[i])
#                 setwd(paste0(dir, "/", fold[i]))
#                 data <- list.files()
#                 wanted <- c(paste0(pr, "m"), paste0(zone, ".ers"), sensor)
#                 matches <- sapply(wanted, grepl, data)
#                 matches.i <- apply(data.frame(matches), 1, function(z) sum(z==TRUE))
#                 matches.i <- matches.i == 2
#                 data.i <- data[matches.i]
                
#                 wanted <- "*10970m&51.ers"
#                 ers.index <- grepl(wanted, data)
#                 data.i<- list.files(pattern=wanted)
#                 data.i<- list.files(pattern=paste('*pre.ers'))
        ...
        write.csv(file=paste(pr,sensor,option,"selected_rainforest_94_extents_2012_for_report.csv", sep="_"), x=results.a)
}
